---
name: huashu-md-html
description: 花叔的多格式导入 / 出版型流水线 skill。三类独有任务：(1) markitdown 把 PDF/DOCX/PPTX/XLSX/HTML/图片/音频/YouTube/EPub/ZIP 等异构来源导入为 markdown；(2) Pandoc + 4 套模板把 markdown 渲染成精美 HTML，含兜底模式（不耗 token，一键套版）和视觉艺术设计师模式（AI 推荐 3 个差异化方向，反 AI slop 审美）；(3) trafilatura + html-to-markdown 把 HTML/URL 归档为 markdown；(4) python-docx 出版型 Word，仅用于「整本书 / book mode / 封面目录页眉页脚 / 出版社送审 / 纸质书定稿 / 投稿」等出版场景。触发词：markitdown、pandoc、trafilatura、book mode、整本书、出版社送审、纸质书定稿、投稿稿、pdf 转 md、pptx 转 md、xlsx 转 md、epub 转 md、youtube 转 md、URL 转 md、md to html、html to md、any to md、生成 html、网页归档、设计师模式、推荐设计方向、出色的 html、定制 html 设计。
---
...
**退出規則 1**：任務是一般 markdown → docx/Word 排版，**且使用者沒明說「出版社送審 / 紙質書 / 投稿 / book mode / 整本書 / 封面目錄頁眉頁腳」**。→ 若環境中有通用的 docx 排版 skill，改用它；沒有就照下表判斷，本 skill 不支援的需求要跟使用者說明。

**退出規則 2a**：任務是評鑑用的固定欄位觀察表，或這類觀察表的分析報告。→ 若環境中有專門產觀察表的 skill，改用它；沒有就照下表判斷，本 skill 不支援的需求要跟使用者說明。

**退出規則 2b**：任務是機構內部給主管審閱的一般報告（非出版品），且要 docx 交付。→ 若環境中有通用的 docx 排版 skill，改用它並選合適的報告版型；沒有就照下表判斷。
...
# 出版社送審單檔（仍要出版型版式，但只有單章/單文件）
python3 scripts/md_to_docx.py article.md --images-dir ./images -o article.docx
